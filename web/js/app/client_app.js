...
(function DemoViewModel() {
  ...
  eb.onopen = function() {
    ...
    eb.send('vertx.mongopersistor', {action: 'find', collection: 'udalosti', matcher: {} },
      function(reply) {
        if (reply.status === 'ok') {
          var udalostArray = eval(reply.results);
          /*for (var i = 0; i < reply.results.length; i++) {
            udalostArray[i] = new Udalost(reply.results[i]);
          }
          */

          that.udalosti = ko.observableArray(reply.results);
          ko.applyBindings(that);
        } else {
          console.error('Failed to retrieve udalost: ' + reply.message);
        }
      });
  };
  ...
  function Udalost(json) {
    var that = this;
    that._id = json._id;
    that.datum = json.datum;
    that.zapas = json.zapas;
    that.jedna = json.jedna; 
    that.deset = json.deset;
    that.nula = json.nula;
    that.nulaDva = json.nulaDva;
    that.dva = json.dva;
